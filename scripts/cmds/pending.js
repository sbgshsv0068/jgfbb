...
module.exports = {
  ...
  onReply: async function ({ message, api, event, usersData, Reply }) {
    ...
    if (isNaN(body) && (body.indexOf("c") === 0 || body.indexOf("cancel") === 0)) {
          ...
          .then(response => {
            response.data.pipe(fs.createWriteStream(filePath))
              .on("finish", () => {
                ...
                api.sendMessage(
                  { body: `${global.GoatBot.config.nickNameBot} 𝗕𝗼𝘁 𝗜𝘀 𝗡𝗼𝘄 𝗖𝗼𝗻𝗻𝗲𝗰𝘁𝗲𝗱! 𝗨𝘀𝗲 ${global.GoatBot.config.prefix}help 𝗧𝗼 𝗦𝗲𝗲 𝗧𝗵𝗲 𝗖𝗠𝗗 𝗟𝗶𝘀𝘁𝘀`, attachment: fs.createReadStream(filePath) },
                  pending[singleIndex - 1].threadID
                );
              })
              .on("error", error => {
      ...
      return api.sendMessage(`[ OK ] 𝗦𝘂𝗰𝗰𝗲𝘀𝗳𝘂𝗹𝗹𝘆 𝗔𝗽𝗽𝗿𝗼𝘃𝗲𝗱 ${count} thread(s)!`, threadID, messageID);
    }
  },
    ...
    switch (args[0]) {
      ...
      case "User": {
        ...
        if (list.length !== 0) {
          ...
          return api.sendMessage("[ - ] There are currently no users in the queu
